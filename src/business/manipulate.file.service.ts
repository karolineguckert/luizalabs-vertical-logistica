...
class ManipulateFileService {
  ...
  public async createPurchasesFromContentOfFile(text: string) {
    ...
    listOfPurchasesText.map(async (purchaseText) => {
      ...
      const userIdExpMatch = this.getExpMatchID(
        initialPurchase.completePurchaseText,
      );
      initialPurchase = this.manipulatePurchaseObject(
        "userId",
        initialPurchase,
        userIdExpMatch,
      );

      const userNameExpMatch = this.getExpMatchUserName(
        initialPurchase.completePurchaseText,
      );
      initialPurchase = this.manipulatePurchaseObject(
        "userName",
      ...
      const orderIdExpMatch = this.getExpMatchID(
        initialPurchase.completePurchaseText,
      );
      initialPurchase = this.manipulatePurchaseObject(
        "orderId",
      ...
      const productIdExpMatch = this.getExpMatchID(
        initialPurchase.completePurchaseText,
      );
      initialPurchase = this.manipulatePurchaseObject(
        "productId",
   ...
   * @private
   */
  private manipulatePurchaseObject(
    fieldName: string,
    initialPurchase: any,
    fieldExpMatch: RegExpMatchArray | null,
  ): any {
    if (fieldExpMatch) {
      const lengthCompletePurchaseText =
        initialPurchase.completePurchaseText.length;
      initialPurchase[fieldName] = fieldExpMatch[0];
      initialPurchase.completePurchaseText =
        initialPurchase.completePurchaseText.slice(
          initialPurchase[fieldName].length,
          lengthCompletePurchaseText,
        );
    }
  ...
  private isAllFieldsWithValue(initialPurchase: any) {
    return (
      initialPurchase.userId.length > 0 &&
  ...
  private setDate(initialPurchase: any) {
    ...
    initialPurchase.date = initialPurchase.completePurchaseText.slice(
      sizeOfDate,
      lengthCompletePurchaseText,
    );
    initialPurchase.completePurchaseText =
      initialPurchase.completePurchaseText.slice(0, sizeOfDate);
